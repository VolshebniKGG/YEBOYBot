...
def create_microphone_icon(size=24, scale=1.0, color=(255,255,255)):
    ...
    # Центрально: щоб мікрофон був у центрі
    # Почнемо від (x, y), десь на середині
    margin = (size - 2*int(10*scale)) // 2  # умовна
...
def create_bubble_icon(width=24, height=24, color=(255,255,255)):
    ...
    d = ImageDraw.Draw(icon)
    # Малюємо хмаринку, що займає майже весь простір, але з невеликими відступами
    pad = 2
...
class RankCog(commands.Cog):
    ...
    def __init__(self, bot: commands.Bot):
        self.bot = bot
        # Запускаємо фонове нарахування голосового XP
    ...
    async def on_message(self, message: discord.Message):
        ...
        if current_text_xp >= xp_needed_text:
            ...
            # Можна повідомити про підвищення рівня
        ...
        while not self.bot.is_closed():
            try:
                for guild in self.bot.guilds:
                    ...
                    # Знаходимо користувачів у voice
                    members_in_voice = []
    ...
    async def rank_command(self, ctx: commands.Context):
        ...
        # Створимо іконки через наші функції
        # (24x24 – приблизний розмір, scale=1.0 – можете підкоригувати)
        ...
        # Вставимо іконку "бульки" (text_icon_img) десь на (100, 50)
        background.paste(text_icon_img, (100, 50), text_icon_img)
        # Потім поряд текст "LVL X", Rank, Total
        ...
        # Зберігаємо у BytesIO та відправляємо
